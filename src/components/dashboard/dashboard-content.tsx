    ...
    return (
        <button
            ...
            className={`flex h-9 items-center gap-2 rounded-md px-4 text-xs outline-none transition-colors focus-visible:ring-2 focus-visible:ring-(--brand) focus-visible:ring-offset-2 ${isActive
                ? "bg-[#FFE8D9] text-(--brand)"
                : "bg-white text-(--brand) hover:bg-[#FFE8D9] hover:cursor-pointer"
                }`}
        >
            {icon}
            {label}
...
function ListView({
    ...
    return (
        <div className="mt-6 rounded-lg border border-[#dfe3e8] bg-white p-5 sm:p-8 md:p-10">
            <div className="flex flex-wrap items-start justify-between gap-6">
                <div>
                    <h2 className="text-lg font-medium text-neutral-950">Mes tâches assignées</h2>
                ...
                <label className="relative block w-full sm:w-64">
                    ...
                    <input
                        ...
                        className="h-15 w-full rounded-md border border-[#dfe3e8] bg-white pr-11 pl-6 text-xs text-neutral-950 outline-none placeholder:text-[#9299a5] placeholder:text-xs focus:border-(--brand) focus:ring-2 focus:ring-[#d3590b26]"
                    />
            ...
            {visibleTaskStatuses.map((status) => {
                ...
                return (
                    <section
                        ...
                        className="min-w-0 rounded-lg border border-[#FFE0E0] bg-white p-4"
                    >
                        <div className="flex items-center gap-3 px-1 py-2">
                            <h2 id={`column-${status}`} className="text-lg font-medium text-[#1F1F1F]">
                                {columnTitle}
                            </h2>
